...
- 左上のXYタイル座標と、制作時にあわせた地図タイルのズームレベルがあればOK
...
# 導入方法・使い方
0. スクリプトを実行したいIllustratorデータの、ズームレベル(z)と左上のタイル座標(x,y)を控えます
1. 本githubの"ai2tiles.jsx"をRAWから名前を付けて保存してください
2. PC内のどこか、もしくはadobe既定のスクリプト保管フォルダに"ai2tiles.jsx"を配置します
3. *スクリプト実行前のIllustratorデータを保存します*
4. Illustratorを起動し、ファイル->スクリプトからPC内にあるai2tiles.jsxを指定、実行します。
5. 入力画面で必要事項を入力してください
    - スクリプトを実行したいデータのz,x,yを入力します
    - 実行後に出力したいズームレベルを4つ目に入力します
6. OKを押し、ファイル選択ダイアログで出力先を指定します
7. しばらく待てば、出力完了！
